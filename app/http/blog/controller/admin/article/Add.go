...
func Add(c *gin.Context) {
	...
	saveData := model.BlogArticle{}
	saveData.Uid = uidD
	saveData.Title = form.Title
	saveData.Content = content
	saveData.Sort = *form.Sort
	saveData.Status = *form.Status
	saveData.CategoryId = form.CategoryId
	err = app.Db("blog").Create(&saveData).Error
	if err != nil {
